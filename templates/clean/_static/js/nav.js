...
$(document).ready(function(){
    NavigationCache[window.location.pathname] = $('html').html();
    history.pushState({page: window.location.pathname, type: "page"}, document.title, window.location.pathname);
});

function Ajax_loadPage(page) {
    $('#content_wrap').load(page + ' #content_wrap', {'isAjax' : true}, function(response) {
        NavigationCache[page] = response;
...
$(document).ready(function() {
    if (history.pushState) {
        window.onpopstate = function(event) {
            if (event.state !== null) {
                if (NavigationCache[event.state.page].length > 0) {
                    $('html').html(NavigationCache[event.state.page]);
                }
            }
        }
        ...
        $('a[target!=_blank]').on('click', function() {
            ...
            if(url_parts.host !== undefined && url_parts.host !== window.location.host)
                return true;

            Ajax_loadPage($(this).attr('href'));
            return false;
        })
    }
});
